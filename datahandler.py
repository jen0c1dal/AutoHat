...
def launch_checkin(data_in_path):
    ...
    mean_throw = raw_data['throws'].mean()
    mean_exp = raw_data['experience'].mean()
    mean_athl = raw_data['athleticism'].mean()
    mean_rank = raw_data['rank'].mean()
    mean_vals = {'throw': [mean_throw], 'exp': [mean_exp], 'athl': [mean_athl], 'rank': [mean_rank]}
    return raw_data, mean_vals
